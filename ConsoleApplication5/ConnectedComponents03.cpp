...
int CalcAngle(Mat orig, Mat src, int componentCount)
{
	...
	myEdgeDetectorFile << "src size= " << src.size() << "\n";
	myEdgeDetectorFile << "Angle size= " << Angle.size() << "\n";
	angle_hist = getHistogram(Angle, "AngleMatrixHist");
	...
	for (size_t i = 0; i < Angle.rows; i++)
	{
		...
		for (size_t j = 0; j < Angle.cols; j++)
		{
			...
			//Mat dst;
			//Angle(Rect(j, i, 1, 1)).convertTo(dst, CV_64F);
			//myEdgeDetectorFile << "src_gray[i=" << i << ",j=" << j << "]=" << dst.at<double>(0) << "\n";
			//myEdgeDetectorFile << "src_gray[i=" << i << ",j=" << j << "]=" << (double)Angle.at<uchar>(j,i) << "\n";
			myEdgeDetectorFile << "src_gray[i=" << i << ",j=" << j << "]=" << Row_i[j] << "\n";

			//vvv[i].at(j) = RowSG_i[j];
			//vvv[int(Row_i[j] / binSize)].push_back(RowSG_i[j]);
			//vvv[int(Row_i[j] / binSize)].push_back((double)Angle.at<uchar>(i, j));
			vvv[int(Row_i[j] / binSize)].push_back(Row_i[j]);
	...
	imshow("Calc Angle src", src);
	ofstream ArrayVector2File;
	ArrayVector2File.open("ArrayVector2File.txt");
	for (int i = 0; i < src.rows; i++)
	{
		const float* sRow_i = src.ptr<float>(i);
		...
		for (int j = 0; j < src.cols; j++)
		{
			if (sRow_i[j]!=0)
			{
				ArrayVector2File << "i= " << i << ", j= " << j << ", value= " << aRow_i[j] << "\n";
			}
		}
	}
	ArrayVector2File.close();
